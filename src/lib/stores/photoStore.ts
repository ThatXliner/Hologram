...
import type { Photo, PhotoFilter, PhotoStats, AppState } from '../types.js';
...
export const filteredPhotos = derived(photoStore, ($store) => $store.filteredPhotos);
export const selectedPhoto = derived(photoStore, ($store) => $store.selectedPhoto);
export const currentFilter = derived(photoStore, ($store) => $store.currentFilter);
